...
static int	ft_conv(char type, va_list ap, int fd, int out_len)
{
	if (type == 'c')
		...
		out_len = ft_putnbr_base(va_arg(ap, unsigned long int), "0123456789abcdef", out_len);
	else if (type == 'x')
		out_len = ft_putnbr_base(va_arg(ap, unsigned long int), "0123456789abcdef", out_len);
	else if (type == 'X')
		out_len = ft_putnbr_base(va_arg(ap, unsigned long int), "0123456789ABCDEFG", out_len);
	else if (type == 'u')
		 out_len = ft_putnbr_base(va_arg(ap, unsigned int), "0123456789", out_len);
	else if (type == '%')
...
int	ft_printf(const char *format ...)
{
	...
	while (*format)
	{
		if (*format == '%')
		{
			format++;
			print_len -= 2;
			print_len += ft_conv(*format, ap, 1, print_len);
